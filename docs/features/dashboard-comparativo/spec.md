...
- Saldo (`balance`): receitas menos despesas. Investimentos não são descontados desse saldo.
...
3. Manter o resumo mensal atual e seu `availableBalance` (receitas menos despesas menos investimentos). O novo `balance` é um contrato distinto, conforme o requisito desta etapa.
...
- **CA02:** as quatro métricas retornam valores atuais, anteriores e diferenças corretas; investimento não reduz `balance`.
